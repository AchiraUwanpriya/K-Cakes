...
const mapSubject = (raw) => {
  ...
  return {
    ...
    // Preserve numeric/financial fields when present on the raw API response
    totalFee:
      raw.totalFee ?? raw.TotalFee ?? raw.fee ?? raw.Fee ?? null,
    duration_In_Months:
      raw.duration_In_Months ?? raw.Duration_In_Months ?? raw.durationMonths ?? raw.duration ?? null,
    monthlyFee:
      raw.monthlyFee ?? raw.MonthlyFee ?? raw.monthly ?? null,
...
export const getNotEnrolledSubjects = async () => {
  const baseUrl = axios?.defaults?.baseURL ?? "";
  const baseHasApiSuffix = /\/api\/?$/i.test(String(baseUrl));

  const candidateEndpoints = new Set([`/Subjects/NotEnrolled`]);
  if (!baseHasApiSuffix) {
    candidateEndpoints.add(`/api/Subjects/NotEnrolled`);
  }
  ...
  for (const endpoint of candidateEndpoints) {
    try {
      ...
      const raw = Array.isArray(resp.data)
        ? resp.data
        : resp.data?.subjects || resp.data?.data || [];
      if (Array.isArray(raw) && raw.length) {
        return raw.map(mapSubject).filter(Boolean);
      }
      // if the endpoint returned empty array or unexpected shape, continue
    } catch (err) {
      lastError = err;
      continue;
    }
  ...
  if (lastError) {
    console.error("Failed to fetch not-enrolled subjects", lastError);
    throw lastError;
  }
