...
export const createEnrollmentPost = async (payload = {}) => {
  ...
  const resolveSubjectId = () => {
    // If caller provided a SubjectID, accept it only if it's a non-zero value
    if (
    ...
    if (payload.SubjectId !== undefined && payload.SubjectId !== null) {
      if (String(payload.SubjectId).trim() !== "" && !(Number(payload.SubjectId) === 0)) {
        return payload.SubjectId;
      }
    }

    // Teacher flow stores the picked class/subject in storage; reuse it if API payload lacks it
    ...
    const storedSubject = readStored("selectedSubjectID");
    if (storedSubject !== null) return storedSubject;

    const storedCourseSubject = readStored("selectedCourseSubjectID");
    if (storedCourseSubject !== null) return storedCourseSubject;
...
export const deleteEnrollment = async (enrollmentId) => {
  ...
  try {
    const resp = await axios.delete(`/Enrollments/${idStr}`);
    if (resp && resp.status && resp.status >= 200 && resp.status < 300) {
      return true;
    }
    return false;
  } catch (err) {
    console.error(`Failed to delete enrollment ${idStr}`, err);
    return false;
  }
};
