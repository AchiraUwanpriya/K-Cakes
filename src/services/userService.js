...
// Activate (reactivate) a previously inactive user
export const activateUser = async (userID) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to activate user: ${response.status} ${txt || response.statusText}`);
  }

  try {
    const payload = await response.json();
    // Some endpoints return { message: "..." } only. Try to fetch fresh user record if possible.
    const updated = await getUserById(userID).catch(() => null);
