...
export const getCourseDetails = async (courseId) => {
  try {
    ...
    // Preserve both formatted fields and raw subject objects so callers
    // that expect detailed subject entries can access them. Some parts
    // of the app use formatCourse() (strings for subjects) while the
    // class picker expects the original subject objects with IDs/fees.
    const formatted = formatCourse(response.data);
    const raw = response.data || {};

    // Prefer raw arrays for the detailed keys so deriveClassOptions and
    // other consumers can find subject objects under multiple possible
    // property names.
    const courseSubjects =
      raw.courseSubjects || raw.CourseSubjects || raw.subjects || raw.Subjects || null;
    const subjectDetails = raw.subjectDetails || raw.SubjectDetails || null;

    const result = {
      ...formatted,
      // attach raw under a dedicated key
      _raw: raw,
      // also expose raw subject collections under common keys
      courseSubjects: courseSubjects,
      CourseSubjects: courseSubjects,
      subjects: courseSubjects || formatted.subjects,
      Subjects: courseSubjects || formatted.subjects,
