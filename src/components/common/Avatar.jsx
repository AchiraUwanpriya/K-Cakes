...
const Avatar = ({
  ...
  useEffect(() => {
    ...
    const loadProfilePhoto = async () => {
      try {
        // Request as a blob so we can handle both JSON metadata and raw image responses.
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
          // Server returned JSON metadata inside a blob; parse it.
          ...
          if (parsed && parsed.hasPicture && parsed.filePath) {
            const normalized = normalizeToAbsoluteUrl(parsed.filePath);
          ...
          profilePictureCache.set(userId, { version: normalizedVersion, src: "" });
          setFetchedSrc("");
          return;
        }

        if (contentType && contentType.startsWith("image/")) {
          // Server returned raw image bytes — create an object URL for display.
          try {
            const objectUrl = URL.createObjectURL(blob);
            // Cache object URL so repeated renders reuse it.
            profilePictureCache.set(userId, {
              version: normalizedVersion,
              src: objectUrl,
            });
            ...
            // Fallthrough to empty state on failure
          }
        }

        // Unknown/empty response — treat as no picture
        profilePictureCache.set(userId, { version: normalizedVersion, src: "" });
        setFetchedSrc("");
      } catch (error) {
        if (!isActive) return;
