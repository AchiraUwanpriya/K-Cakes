...
const CourseForm = ({
  ...
  const {
    ...
    defaultValues: {
      ...initialData,
      academicYear: initialData?.academicYear ?? computeAcademicYear(),
    },
  });
  ...
  }, [unselectedAvailableSubjects, classSearchQuery]);

  const handleSelectClass = (found) => {
    if (!found) return;
    const name = found.name ?? found.subjectName ?? found.SubjectName ?? "";
    if (
  ...
  const loadAvailableSubjects = async () => {
    ...
    try {
      const subs = await getNotEnrolledSubjects();
    ...
    >
      ...
      <div>
        ...
        <div className="mt-2 space-y-2">
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-2 sm:items-center w-full min-w-0">
            <div className="relative w-full sm:flex-1 min-w-0" ref={classDropdownRef}>
              <button
                type="button"
                onClick={() =>
              ...
              >
                <span className="truncate text-gray-500 dark:text-gray-400 text-sm">
                  ...
                  <div className="max-h-56 sm:max-h-60 overflow-y-auto p-1 divide-y divide-gray-50 dark:divide-gray-800/40 custom-scrollbar">
                    {filteredAvailableSubjects.length === 0 ? (
                      <div className="py-5 text-center text-xs sm:text-sm text-gray-400 dark:text-gray-500">
                        {unselectedAvailableSubjects.length === 0
                      ...
                      filteredAvailableSubjects.map((s) => {
                        const name =
                          s.name ?? s.subjectName ?? s.SubjectName ?? "";
                        return (
                          <button
                            key={String(s.id)}
                            type="button"
                            onClick={() => handleSelectClass(s)}
                            className="w-full text-left px-3 py-2 text-xs sm:text-sm rounded-lg transition hover:bg-gray-100 dark:hover:bg-gray-700/60 text-gray-700 dark:text-gray-200 flex items-center justify-between"
                          >
                            <span className="truncate">{name}</span>
                            <span className="text-[11px] font-medium text-indigo-600 dark:text-indigo-400 ml-2 flex-shrink-0">
                              + Add
                            </span>
                          </button>
                        );
            ...
            <div className="w-full sm:w-auto flex-shrink-0">
              <Button
                ...
                className="w-full justify-center sm:w-auto h-10 sm:h-auto text-xs sm:text-sm"
              >
                + New
              </Button>
            </div>
