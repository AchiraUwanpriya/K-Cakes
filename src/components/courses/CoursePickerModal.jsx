...
const [selectedCourses, setSelectedCourses] = useState([]);
  ...
  const filtered = useMemo(() => {
    ...
    let visible = (courses || []).filter((course) => {
      ...
      return cid && !excludeSet.has(cid) && isCourseActive(course);
    });
      ...
      if (missingIds.length) {
        const missingCourses = (courses || []).filter((course) => {
          ...
          if (missingCourses.length) {
            // only restore missing courses that are active
            const activeMissing = missingCourses.filter(isCourseActive);
            visible = [...visible, ...activeMissing];
          }
      }
    }
