...
// Safely resolve a course-level subject label — course.subject can be
// a string, an object, or an array. Return a plain string for rendering.
const resolveCourseSubjectLabel = (course) => {
  if (!course) return "";
  ...
  for (const val of candidates) {
    ...
    if (Array.isArray(val) && val.length) {
      ...
      if (first && typeof first === "object") {
        return (
          first.subjectName || first.SubjectName || first.name || first.Name || ""
        );
      }
      continue;
    }
    ...
    if (typeof val === "object") {
      const t =
        val.subjectName || val.SubjectName || val.name || val.Name || null;
