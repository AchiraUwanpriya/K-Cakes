...
const deriveClassOptions = (
  ...
  const registerOption = (rawId, labelCandidate, extra = {}) => {
    ...
    // attach fee metadata when available (prefer values passed in `extra`)
    ...
    option.monthlyFee =
      resolvedMonthly !== undefined ? resolvedMonthly : option.monthlyFee;
    option.totalFee =
      resolvedTotal !== undefined ? resolvedTotal : option.totalFee;
...
const AdminUsers = () => {
  ...
  const loadClassOptionsForCourse = async (selectedCourseId) => {
    ...
    try {
      // Always try to fetch fresh formatted course details first
      courseDetails = await getCourseDetails(normalizedId);
    } catch (error) {
      console.warn("Failed to load course details for class selection", error);
    }

    // If formatted details don't include rich subject objects, try fetching
    // the raw course payload from the API so deriveClassOptions can read
    // subject IDs and other metadata. If network calls fail, fall back to
    // previously cached options (if any) to avoid breaking the flow.
    try {
      const hasObjectSubjects =
        Array.isArray(courseDetails?.subjects) &&
        courseDetails.subjects.length &&
        typeof courseDetails.subjects[0] === "object";
      const hasCourseSubjects =
        Array.isArray(courseDetails?.courseSubjects) &&
        courseDetails.courseSubjects.length;
      const hasSubjectDetails = Boolean(
        courseDetails?.subjectDetails || courseDetails?.SubjectDetails
      );

      if (!hasObjectSubjects && !hasCourseSubjects && !hasSubjectDetails) {
        try {
          const resp = await axios.get(`/Courses/${normalizedId}`);
          if (resp && resp.data) {
            courseDetails = resp.data;
          }
        } catch (rawErr) {
          console.warn(
            "Failed to fetch raw course details; will use cache if available",
            rawErr
          );
          if (!courseDetails && cached) {
            return cached;
          }
        }
      }
  ...
  const handleStudentCourseSelectionChange = async (
    ...
    for (const courseIdCandidate of coursesToProcess) {
      ...
      // Check if we already have a selection for this course
      const existingSelection = workingMap[courseId]
        ? [String(workingMap[courseId].subjectId)]
        : [];
      // Show class picker for ALL courses, not just new ones
  ...
  const handleClassPickerProceed = async (selectedSubjectIds) => {
    if (classPickerContext?.mode === "edit-student-course") {
      ...
      try {
        const chosenId = normalized[0] ?? null;
        const normalizedChosen = chosenId ? normalizeIdString(chosenId) : null;
        const courseIdCandidate = classPickerContext?.courseId || null;
        const normalizedCourseId = normalizeIdString(courseIdCandidate);
        if (normalizedChosen) {
          const optionMap = new Map(
            (classOptions || []).map((opt) => [normalizeIdString(opt.id), opt])
          );
          const matched = optionMap.get(normalizedChosen) || null;
          const monthly = matched?.monthlyFee ?? matched?.MonthlyFee ?? null;
          const total = matched?.totalFee ?? matched?.TotalFee ?? null;
          if (monthly !== null && monthly !== undefined) {
            localStorage.setItem("selectedMonthlyFee", String(monthly));
          }
          if (total !== null && total !== undefined) {
            localStorage.setItem("selectedTotalFee", String(total));
          }
          ...
          localStorage.setItem("selectedSubjectID", String(normalizedChosen));
        }
      } catch (err) {
        console.warn("Failed to persist selected class fees:", err);
      }
    ...
    // Persist the fees and selected ids for the chosen class so payment flow
    // uses the actual class (subject) the admin picked rather than a default
    // first-subject value from the course details.
    try {
      const chosenId = normalized[0] ?? null;
      const normalizedChosen = chosenId ? normalizeIdString(chosenId) : null;
      const courseIdCandidate =
        classPickerContext?.courseId ||
      ...
      if (normalizedChosen) {
        ...
        if (normalizedCourseId) {
          localStorage.setItem("selectedCourseID", String(normalizedCourseId));
        }
        localStorage.setItem("selectedSubjectID", String(normalizedChosen));
      }
    } catch (err) {
  ...
  const handleUserSubmit = async (userData) => {
    try {
      ...
      if (!selectedUser) {
          ...
          try {
            ...
            const studentPayload = {
              UserID:
                ...
                createdUser.userId,
              RollNumber:
                mergedCreate.RollNumber ??
                mergedCreate.IDNumber ??
                mergedCreate.rollNumber ??
                mergedCreate.idNumber ??
                undefined,
              EnrollmentDate: enrollmentDateValue ?? undefined,
              CurrentGrade:
                mergedCreate.CurrentGrade ??
                mergedCreate.Class ??
                mergedCreate.currentGrade ??
                mergedCreate.class ??
                undefined,
              ParentName:
                mergedCreate.ParentName ??
                mergedCreate.GuardianName ??
                mergedCreate.parentName ??
                mergedCreate.guardianName ??
                undefined,
              ParentContact:
                mergedCreate.ParentContact ??
                mergedCreate.GuardianPhone ??
                mergedCreate.parentContact ??
                mergedCreate.guardianPhone ??
                undefined,
            };
            ...
            try {
              ...
              setFormError(
                studentErr?.message ||
                  "Student created but failed to save details"
              );
            }
            ...
            if (createdStudent && typeof createdStudent === "object") {
              const studentIdCandidate =
                ...
                null;
              if (
                studentIdCandidate !== null &&
                studentIdCandidate !== undefined
              ) {
                enrichedUser.StudentID = studentIdCandidate;
                enrichedUser.studentID = studentIdCandidate;
              }
        ...
        try {
          if (typeId === "3") {
            // Enrollment count validation removed so sync logic can create/update/delete
            // enrollments as needed. The following code will handle alignment.
            ...
            try {
              ...
              const currentEnrollmentCount = (enrollmentList || []).length;

              const selectedRaw = userData.StudentCourseIDs || [];
              ...
              const desiredCount = selected.length;

              // Validation: Counts must match
              if (currentEnrollmentCount !== desiredCount) {
                setFormError(
                  `User has ${currentEnrollmentCount} enrollment(s). You have selected ${desiredCount} course(s). ` +
                    `Please select exactly ${currentEnrollmentCount} course(s) to match existing enrollments.`
                );
              ...
              const currentEnrollments = (enrollmentList || [])
                ...
                .map((enrollment) => {
                  ...
                  //   enrollment.Course.subjectObj.id??
                  //   enrollment.Course.subjectID??
                  //   enrollment.Course.subjects.raw?.SubjectID??
                  //   enrollment.raw?.subjectID
                  // );
  ...
  const handleActivateUser = async (userID) => {
    try {
      setFormError("");
      // call API to activate the user (backend PUT /Activate/:id)
  ...
  const handleDeactivateUser = async (userID) => {
    try {
      setFormError("");
      // Use DELETE endpoint which moves user to inactive
      // Confirm before deactivating
      // eslint-disable-next-line no-restricted-globals
      const ok = window.confirm(
        "Are you sure you want to move this user to inactive?"
      );
      if (!ok) return;
      await deleteUser(userID);
      // After delete (moved to inactive) refresh lists in memory: mark user as inactive or remove from active list
      setUsers((prev) =>
        prev.map((u) => {
          const id = u.UserID || u.id || u.userID || u.userId || null;
          if (String(id) === String(userID)) {
            return { ...u, IsActive: false, isActive: false };
          }
          return u;
        })
      );
        ...
        return (
          <>
            ...
            {(() => {
              ...
              const activeUsers = (searchFiltered || []).filter(isActiveFlag);
              const inactiveUsers = (searchFiltered || []).filter(
                (u) => !isActiveFlag(u)
              );
